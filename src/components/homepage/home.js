...
class Home extends Component {
    ...
    render() {
        ...
        return (
            ...
            <div>
                {/*onMouseOut={() => console.log('mouse out')} onMouseOver={() => {
                    console.log('mouse hovered')
                }}*/}
                <div className="marginTop30 marginBottom100" align="center"  onClick={() => {
                    let elem = document.getElementById('test-id')
                    let inputElem = document.getElementById('input-id')
                    elem.click();
                    inputElem.focus();
                    /*   console.log('clicked')
                       let elem = document.getElementById('test-id')
                       ...
                       elem.style.margin = "2em";ind
                       elem.classList.add("test-class")
                       console.log(elem)
                       console.log(elem.classList)
                       console.log(new Date(Date.now()).getDay())
                       if (new Date(Date.now()).getDay() === 3) {
                           elem.innerHTML = "Tuesday"

                       }
                       console.log(elem)*/
                }} style={{color: "blue"}} id="test-id">
                    <div id={"input-wrap"}>
                        <input name="test-input" onClick={()=>{
                            document.getElementById('input-id').style.fontSize="20px"
                        }} onDoubleClick={()=>console.log('double clicked')} onInput={(event)=>{
                            this.handleChange(event)
                        }} className="test-class-0" id="input-id"/>
                    </div>
                </div>
                ...
                {/*</div>*/}
                <div className="commonWrapper" style={{gridTemplateColumns: "repeat(5,1fr)", margin: 100}}>
                ...
                <div className="countryWrapper" style={{gridTemplateColumns: "repeat(5,1fr)", margin: 100}}>
                    ...
                    {countries.filter((country) => this.state.searchKey && !country.isFavourite ? country.name.toUpperCase().includes(this.state.searchKey.toUpperCase()) : !country.isFavourite).map((country, index) =>
                        <CountryCard array={country.name.charAt(0) === 'N' ? this.state.customArray : []}
                                     updateCountry={() => this.updateList()} key={index} country={country}/>)
                    }
